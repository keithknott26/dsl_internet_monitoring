...
class DSLStats:
    ...
    link1_attainable_rate: LineStat = LineStat(
        stat_name="link1_attainable_rate", downstream_value=0.0, upstream_value=0.0
    ) 
    link2_attainable_rate: LineStat = LineStat(
        stat_name="link2_attainable_rate", downstream_value=0.0, upstream_value=0.0
    )
    link1_sync_rate: LineStat = LineStat(
        stat_name="link1_sync_rate", downstream_value=0.0, upstream_value=0.0
    ) 
    link2_sync_rate: LineStat = LineStat(
        stat_name="link2_sync_rate", downstream_value=0.0, upstream_value=0.0
    )      
    link1_rs_uncorrectable_errors: LineStat = LineStat(
        stat_name="link1_rs_uncorrectable_errors", downstream_value=0.0, upstream_value=0.0
    ) 
    link2_rs_uncorrectable_errors: LineStat = LineStat(
        stat_name="link2_rs_uncorrectable_errors", downstream_value=0.0, upstream_value=0.0
    )           
    link1_rs_correctable_errors: LineStat = LineStat(
        stat_name="link1_rs_correctable_errors", downstream_value=0.0, upstream_value=0.0
    ) 
    link2_rs_correctable_errors: LineStat = LineStat(
        stat_name="link2_rs_correctable_errors", downstream_value=0.0, upstream_value=0.0
    )         
...
def get_stats() -> str:
    r = requests.get("http://192.168.254.254/cgi-bin/dslstatistics.ha", timeout=10)
    return r.text
...
def parse_stats() -> DSLStats:
    try:
        ...
        print("NO STATS RETURNED / NETWORK UNREACHABLE")
    ...
    return DSLStats(
        ...
        #kknott added
        link1_attainable_rate=st.find_line_stat_by_name("Attainable Rate (kbps)", 0),
        link2_attainable_rate=st.find_line_stat_by_name("Attainable Rate (kbps)", 1),

        link1_rs_correctable_errors=st.find_line_stat_by_name("RS Correctable Errors", 0),
        link2_rs_correctable_errors=st.find_line_stat_by_name("RS Correctable Errors", 1),
        
        link1_rs_uncorrectable_errors=st.find_line_stat_by_name("RS Uncorrectable Errors", 0),
        link2_rs_uncorrectable_errors=st.find_line_stat_by_name("RS Uncorrectable Errors", 1),
...
def poll_stats(step_seconds: int = 1):
    ...
    connection_state = Enum("connection_state", "WAN Connection", states=["Connected", "Offline", "Down"])
    #kknott
    attainable_rate = Gauge("attainable_rate_kbps", "Attainable Rate (kbps)", ["link", "direction"])
    rs_correctable_errors = Gauge("rs_correctable_error_count", "RS Correctable Errors", ["link", "direction"])
    rs_uncorrectable_errors = Gauge("rs_uncorrectable_error_count", "RS Uncorrectable Errors", ["link", "direction"])
    superframe_errors = Gauge("superframe_error_count", "Super Frame Errors", ["link", "direction"])
    ...
    while True:
        ...
        if stats is not None:
            ...
            attainable_rate.labels("Link 1", "Downstream").set(stats.link1_attainable_rate.downstream_value)
            attainable_rate.labels("Link 1", "Upstream").set(stats.link1_attainable_rate.upstream_value)
            attainable_rate.labels("Link 2", "Downstream").set(stats.link2_attainable_rate.downstream_value)
            attainable_rate.labels("Link 2", "Upstream").set(stats.link2_attainable_rate.upstream_value)
        
            rs_correctable_errors.labels("Link 1", "Downstream").set(stats.link1_rs_correctable_errors.downstream_value)
            rs_correctable_errors.labels("Link 1", "Upstream").set(stats.link1_rs_correctable_errors.upstream_value)
            rs_correctable_errors.labels("Link 2", "Downstream").set(stats.link2_rs_correctable_errors.downstream_value)
            rs_correctable_errors.labels("Link 2", "Upstream").set(stats.link2_rs_correctable_errors.upstream_value)
                        
            rs_uncorrectable_errors.labels("Link 1", "Downstream").set(stats.link1_rs_correctable_errors.downstream_value)
            rs_uncorrectable_errors.labels("Link 1", "Upstream").set(stats.link1_rs_uncorrectable_errors.upstream_value)
            rs_uncorrectable_errors.labels("Link 2", "Downstream").set(stats.link2_rs_uncorrectable_errors.downstream_value)
            rs_uncorrectable_errors.labels("Link 2", "Upstream").set(stats.link2_rs_uncorrectable_errors.upstream_value)
            
            superframe_errors.labels("Link 1", "Downstream").set(stats.link1_superframe_errors.downstream_value)
            superframe_errors.labels("Link 1", "Upstream").set(stats.link1_superframe_errors.upstream_value)
            superframe_errors.labels("Link 2", "Downstream").set(stats.link2_superframe_errors.downstream_value)
            superframe_errors.labels("Link 2", "Upstream").set(stats.link2_superframe_errors.upstream_value)

            hec_errors.labels("Link 1", "Downstream").set(stats.link1_hec_errors.downstream_value)
            hec_errors.labels("Link 1", "Upstream").set(stats.link1_hec_errors.upstream_value)
            hec_errors.labels("Link 2", "Downstream").set(stats.link2_hec_errors.downstream_value)
            hec_errors.labels("Link 2", "Upstream").set(stats.link2_hec_errors.upstream_value)

            total_err_seconds.labels("Link 1", "Downstream").set(stats.link1_total_err_seconds.downstream_value)
            total_err_seconds.labels("Link 1", "Upstream").set(stats.link1_total_err_seconds.upstream_value)
            total_err_seconds.labels("Link 2", "Downstream").set(stats.link2_total_err_seconds.downstream_value)
            total_err_seconds.labels("Link 2", "Upstream").set(stats.link2_total_err_seconds.upstream_value)

            sn_margin_db.labels("Link 1", "Downstream").set(stats.link1_sn_margin_db.downstream_value)
            sn_margin_db.labels("Link 1", "Upstream").set(stats.link1_sn_margin_db.upstream_value)
            sn_margin_db.labels("Link 2", "Downstream").set(stats.link2_sn_margin_db.downstream_value)
            sn_margin_db.labels("Link 2", "Upstream").set(stats.link2_sn_margin_db.upstream_value)
